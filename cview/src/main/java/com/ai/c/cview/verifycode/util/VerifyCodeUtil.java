package com.ai.c.cview.verifycode.util;
...
//              if ((t >= 65 || (t >= 48 && t <= 57)) && (exChars == null || exChars.indexOf((char) t) < 0)) {
                if (((t >= 65 && t != 79 && t != 73) || (t > 48 && t <= 57)) && (exChars == null || exChars.indexOf((char) t) < 0)) {
                    code.append((char) t);
                    i++;
                }
            }
    ...
    public static BufferedImage generateImageCode(String textCode, int width, int height, int interLine, boolean randomLocation, Color backColor,
        ...
        int fy = fsize;
        //设置字体
        g.setFont(new Font("", Font.BOLD, fsize));

        // 写验证码字符
        for (int i = 0; i < textCode.length(); i++) {
            fy = randomLocation ? (int) ((Math.random() * 0.3 + 0.8) * height) : fy;// 每个字符高低是否随机
